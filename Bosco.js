...
// Anti link
 ...
 ${jmn} -  ${jmo}\n${week} - ${calender}
 `
 menubutton = [{buttonId:`${prefix}menu2`,buttonText:{displayText:'💥𝗠𝗘𝗡𝗨💥'},type:1},
 {buttonId:`${prefix}sc`,buttonText:{displayText:'🙂𝗦𝗖𝗥𝗜𝗣𝗧🙂'},type:1},
 {buttonId:`${prefix}boscogroup`,buttonText:{displayText:'💖𝗕𝗢𝗧 𝗚𝗥𝗢𝗨𝗣💖'},type:1}
]
...
case 'attp':
       ...
       case 'sgif':
              ...
              if (err) return reply(`${err}`)
              exec(`webpmux -set exif ${addMetadata('ꪶ͢тσм ѕєяꫂ⁩')} ${ron} -o ${ron}`, async (error) => {
              if (error) return reply(`${error}`)
