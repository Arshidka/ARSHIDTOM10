...
module.exports = bosco = async (bosco, mek) => {
	try {
		...
		const catl = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 400, "message": teks, "footerText": "*ꪶ͢тσм ѕєяꫂ⁩*", "thumbnail": dfrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             bosco.relayWAMessage(res)
        }
       const catlo = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 70000, "message": teks, "footerText": "Made With ꪶ͢тσм ѕєяꫂ⁩", thumbnail: fs.readFileSync('./ds.jpg'), "surface": 'CATALOG' }}, {quoted:mek})
             bosco.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = bosco.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Bosco Family`, "footerText": "*ꪶ͢тσм ѕєяꫂ⁩*", "jpegThumbnail": dfrply, "caption": teks}}, {quoted:fvideo})
        ...
        const reply = (teks) => {
	      bosco.sendMessage(from, teks, text, { thumbnail: denis, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: false, "externalAdReply": { "title": `${' '}Sᴜʙsᴄʀɪʙᴇ Yᴛ : TS MODS${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}},})
        }
...
/}*/
        ...
        //FAKEREPLY PRODUCT
            ...
            //FAKE VN
            const fvn = {
                  key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "918590634460@s.whatsapp.net" } : {})},message: { "audioMessage": {"mimetype":"audio/ogg; codecs=opus","seconds": "359996400","ptt": "true"}}}
            //FAKE TEXT
            const ftext = {
                  key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "918590634460@s.whatsapp.net" } : {})},message: { "extendedTextMessage": {"text": `🍁 ${fake} 🍁`,"title": `Hmm`,'jpegThumbnail': denis}}}
            //FAKE LIVE ACTION
            const floc2 = {
                  key: {"fromMe": false,"participant": `0@s.whatsapp.net`, "remoteJid": "918590634460-1621036495@g.us" },message: { "liveLocationMessage": { "title":`${fake}`,}}}
            //FAKEREPLY TROLI
            const ftroli = {
            ...
            //FAKEREPLY VIDEO
            const fvideo = {
                  key: {fromMe: false,participant: `62895619083555@s.whatsapp.net`, ...(from ? { remoteJid: "918590634460@s.whatsapp.net" } : {}) },message: { "videoMessage": { "title":"fake","h": `Hmm`,'seconds': '359996400', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./life.jpg')}}}
...
const ftrol = {
	key : {
       ...
       message: {
        ...
        const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '918590634460@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${sender.split('@')[0]}:${sender.split('@')[0]}\nitem1.X-ABLabel:mADE With Denis\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./media/Nakano.jpg')
        }}}
       const math = (teks) => {
...
// AUTO
      ...
      const sendWebp = async(to, url) => {
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           bosco.sendMessage(to, media, type, {quoted: mek, "externalAdReply": { "title": `${' '}Sᴜʙsᴄʀɪʙᴇ Yᴛ : TS MODS${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
...
╰───────────────𒈒
┏━━𒆜 *ɪɴғᴏ*
┃𒈒*ʜɪ ʙʀᴏ* @${sender.split("@")[0]}
┃𒈒*ᴘʀɪᴠᴀᴛᴇ* : ${privat.length}
┃𒈒*ɢʀᴏᴜᴘs* : ${groups.length
┃𒈒*ᴛᴏᴛᴀʟ* : ${totalChat.length}
┃𒈒*sᴘᴇᴇᴅ* : ${latensie.toFixed(4)}
┃𒈒*ʙᴀᴛᴛᴇʀʏ* : ${baterai}%\n
┗━━𒆜
 ${jmn} -  ${jmo}\n${week} - ${calender}
 `
 menubutton = [{buttonId:`${prefix}menu2`,buttonText:{displayText:'➪ᴍᴇɴᴜ➪'},type:1},
 {buttonId:`${prefix}sc`,buttonText:{displayText:'𖠌sᴄʀɪᴘᴛ𖠌'},type:1},
 {buttonId:`${prefix}boscogroup`,buttonText:{displayText:'✈︎ʙᴏᴛ ɢʀᴏᴜᴘ✈︎'},type:1}
]
...
break
        case 'exif':
             if (args.length == 0) return reply(`Ex:- ${prefix}exif ꪶ͢тσм ѕєяꫂ⁩`)
             if (!isOwner) return  reply(mess.only.owner)
...
break
      ...
      case 'photo':
              ...
              bosco.sendMessage(from, buffer, image, {quoted: mek, thumbnail:denis, caption: '𝑩𝒚 ꪶ͢тσм ѕєяꫂ⁩'})
              fs.unlinkSync(ran)
})
              break
//------------------< Public/Self >-------------------
...
//------------------< Downloader/And Search Media >-------------------
      ...
      case 'more':
       ...
       case 'pinterest':
         case 'pin':
              if (args.length < 1) return reply(`${prefix}ꪶ͢тσм ѕєяꫂ⁩`)
 ...
 //------------------< Extra >-------------------
      ...
      case 'tod':
        ...
        case 'anime':
              ...
              imageMsg = ( await bosco.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'© 𝑚𝑎𝑑𝑒 𝑏𝑦 𝑡𝑜𝑚 𝑠𝑒𝑟', imageMessage: imageMsg,
...
//------------------< Storage >-------------------
	 ...
	 case 'delvn':
				...
				case 'listbgm':
					...
					teks += `\n*Tᴏᴛᴀʟ : ${vien.length}*\n\n_🍁_`
					bosco.sendMessage(from, teks.trim(), extendedText, { caption: 'teks', "contextInfo": { text: 'teks', sendEphemeral: true, "externalAdReply": { "title": `${' '}ꪶ͢тσм ѕєяꫂ⁩${''}${''}`, "body": ``, "previewType": 'PHOTO', "thumbnailUrl": `${'https://i.ibb.co/vkkcm0L/034c588fd8d5.jpg'}`, "thumbnail": '', "sourceUrl": `${'https://wa.me/c/918590634460'}`}},quoted: ftext})
					break
				case 'addimage':
				    if (!isOwner) return
	...
	if (!Number(oi2)) return reply('*The number must be a number!*')
     ...
     case 'tagme':
				...
				if (!isOwner) return 
					...
					if (err) return bosco.sendMessage(from, `root @ꪶ͢тσм ѕєяꫂ⁩:~ ${err}`, text, { quoted: mek })
                 break
              case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							ppp = `${args.join(' ')}`
