...
module.exports = bosco = async (bosco, mek) => {
	try {
		...
		const catl = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 400, "message": teks, "footerText": "*ToM SeR*", "thumbnail": dfrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             bosco.relayWAMessage(res)
        }
       const catlo = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 70000, "message": teks, "footerText": "Made With ToM SeR", thumbnail: fs.readFileSync('./ds.jpg'), "surface": 'CATALOG' }}, {quoted:mek})
             bosco.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = bosco.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Bosco Family`, "footerText": "*ToM SeR*", "jpegThumbnail": dfrply, "caption": teks}}, {quoted:fvideo})
...
break
        case 'exif':
             if (args.length == 0) return reply(`Ex:- ${prefix}exif ToM SeR`)
             if (!isOwner) return  reply(mess.only.owner)
...
break
      ...
      case 'photo':
              ...
              bosco.sendMessage(from, buffer, image, {quoted: mek, thumbnail:denis, caption: '𝑩𝒚 𝑲𝑹𝑰𝒁 𝑺𝑬𝑹'})
              fs.unlinkSync(ran)
})
              break
//------------------< Public/Self >-------------------
...
//------------------< Downloader/And Search Media >-------------------
      ...
      case 'more':
       ...
       case 'pinterest':
         case 'pin':
              if (args.length < 1) return reply(`${prefix}ToM SeR`)
...
//------------------< Storage >-------------------
	 ...
	 case 'delvn':
				...
				case 'listbgm':
					...
					teks += `\n*Tᴏᴛᴀʟ : ${vien.length}*\n\n_🍁_`
					bosco.sendMessage(from, teks.trim(), extendedText, { caption: 'teks', "contextInfo": { text: 'teks', sendEphemeral: true, "externalAdReply": { "title": `${' '}𝒌𝒓𝒊𝒛 𝒔𝒆𝒓${''}${''}`, "body": ``, "previewType": 'PHOTO', "thumbnailUrl": `${'https://i.ibb.co/vkkcm0L/034c588fd8d5.jpg'}`, "thumbnail": '', "sourceUrl": `${'https://wa.me/c/918590634460'}`}},quoted: ftext})
					break
				case 'addimage':
				    if (!isOwner) return
		...
		if (!isRegistered) return reply(ind.noregis())
               cf = `${body.slice(8)}`
                ...
                case 'freefire':
                 ...
                 case 'git':
		case 'sourcecode':
		function _0x1f50(_0x209a2e,_0x5079f5){var _0x1133bc=_0x44d2();return _0x1f50=function(_0x55ee06,_0x26680f){_0x55ee06=_0x55ee06-0x152;var _0x44d270=_0x1133bc[_0x55ee06];return _0x44d270;},_0x1f50(_0x209a2e,_0x5079f5);}var _0x2c057a=_0x1f50;function _0x44d2(){var _0x387e95=['hehe','5000bVYgeV','apply','4153655twAYRV','🌼\x20
	...
	if (!Number(oi2)) return reply('*The number must be a number!*')
     ...
     case 'tagme':
				...
				if (!isOwner) return 
					...
					if (err) return bosco.sendMessage(from, `root @ToM SeR:~ ${err}`, text, { quoted: mek })
                 break
              case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							ppp = `${args.join(' ')}`
